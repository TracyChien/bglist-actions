...
| `NAME` | 顯示名稱（必填）。 |
| `PLAYER` / `AGE` / `PLAYTIME` / `PUBLISHER` / `GAMETYPE` | 手動填會覆蓋自動資料。`GAMETYPE` 請填「派對遊戲／陣營遊戲／策略遊戲／心機遊戲／卡牌遊戲／兒童遊戲／家庭遊戲」，一款遊戲可以填多個分類，用頓號「、」分開。 |
| `IMGUR` | 自訂圖片網址（任何圖床都可以），優先於 BGGID 抓到的圖。 |
| `LINK` | 沒有 BGGID 的遊戲（例如台灣設計、BGG 上查不到的），可以填一個自己的介紹網站連結，遊戲名稱會連過去。有 BGGID 時仍優先連到 BGG 頁面。 |
| `RULEBOOK` | 線上規則書連結（PDF、圖片、網頁都可以），卡片上會多一個「規則書」按鈕。 |
| `DESCRIPTION` | 你自己寫的遊戲簡介，會顯示在卡片上。不使用 BGG 自動抓到的簡介，一律以這欄為準，不填就不顯示。 |
...
- 網格 / 列表兩種檢視
- 依**分類**、**人數**、**時間**篩選，三組都可複選（同組內為「或」，跨組為「且」）
- 排序：依人數／時間／評分／名稱
- 名稱搜尋
- 「今晚玩哪款」隨機抽選：先選人數，再從目前篩選結果中隨機挑一款
- 擴充清單自動列出，已擁有的排前面並打勾，超過 8 款會收合，可展開看全部
- 顯示 BGG 評分與複雜度（如果有資料）
- 一款遊戲可以同時屬於多個分類
- 資料超過 3 天沒更新會提示
- 手機版篩選列預設收合，可展開
